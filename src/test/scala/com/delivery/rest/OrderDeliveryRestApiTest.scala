 ...
 */
class OrderDeliveryRestApiTest extends FlatSpec with Matchers with ScalatestRouteTest {
  ...
  "Couriers rest api" should "create courier via POST request" in {
    ...
    ) ~> OrderDeliveryRestAPI.routes ~> check {
  ...
  it should "return entity by id via GET request" in {
    Get("/v1/couriers/1") ~> OrderDeliveryRestAPI.routes ~> check {
      status shouldEqual StatusCodes.OK
      responseAs[Courier] shouldBe testCourier
    }

    Get("/v1/couriers/3") ~> Route.seal(OrderDeliveryRestAPI.routes) ~> check {
      status shouldEqual StatusCodes.NotFound
    }
  }

  it should "return all entities via GET request" in {
    Get("/v1/couriers") ~> OrderDeliveryRestAPI.routes ~> check {
      status shouldEqual StatusCodes.OK
  ...
  it should "return entity by id via GET request" in {
    Get("/v1/orders/1") ~> OrderDeliveryRestAPI.routes ~> check {
      status shouldEqual StatusCodes.OK
      responseAs[Order] shouldBe testOrder
    }

    Get("/v1/orders/3") ~> Route.seal(OrderDeliveryRestAPI.routes) ~> check {
      status shouldEqual StatusCodes.NotFound
    }
  }

  it should "return all entities via GET request" in {
    Get("/v1/orders") ~> OrderDeliveryRestAPI.routes ~> check {
      status shouldEqual StatusCodes.OK
  ...
  "Assignments rest api" should "show right assignments via GET request" in {
    Get("/v1/assignments") ~> OrderDeliveryRestAPI.routes ~> check {
  ...
  "Retry Assignments rest api" should "show right assignments via GET request" in {
    ...
    Get("/v1/retryAssignments") ~> OrderDeliveryRestAPI.routes ~> check {
      println(responseAs[List[Assignment]])
      status shouldEqual StatusCodes.OK
      responseAs[List[Assignment]] shouldBe
